# OSINT Coordinated Inauthentic Behavior Detector

Detects coordinated inauthentic behavior (CIB) in social media datasets. Given a collection of posts from multiple accounts, identifies clusters of accounts operating as a coordinated network.

Unlike persona-matching tools that ask "are these the same person?", CIB detection asks **"are these accounts acting in concert?"** — the operators may be different people working from a shared playbook or troll farm.

---

## how it works

Five signal layers, combined into a weighted coordination graph:
...
**content coordination** (D3lta + sentence-transformers)
...
- posting interval regularity (unnaturally even = suspicious)
...
**D3lta integration** — optionally uses [VIGINUM's D3lta library](https://github.com/VIGINUM-FR/D3lta) for content duplication detection. D3lta is an open-source tool built by France's foreign digital interference agency (VIGINUM) that classifies duplicate content as copypasta (near-exact copies), rewording (same meaning, different phrasing), or translation (same content across languages). When installed (`pip install git+https://github.com/VIGINUM-FR/D3lta.git`), D3lta replaces the built-in content similarity engine with Faiss-accelerated semantic search and cross-language matching. The tool works without D3lta using sentence-transformers as a fallback.
